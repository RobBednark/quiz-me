...
def _get_flashcard(request, form_flashcard=None):
    ...
    if not form_flashcard:
        form_flashcard = FormFlashcard(initial=dict(hidden_question_id=next_question.question.id))
    if next_question.question:
        question_tag_names = ", ".join(
            sorted([
                str(
...
def _NEW_create_and_get_usertags(request):
    ...
    if request.method == 'GET':
        ...
        # NOTE/BUG: if there is a new tag, this won't see it, because date=request.POST does not see the new tag
        modelformset_usertag = ModelFormset_UserTag(
            queryset=queryset,
            data=request.POST
        )
