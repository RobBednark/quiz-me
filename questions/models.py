...
class Tag(CreatedBy):
    '''
        ...
        Tag.children.all()  (ParentTag2ChildTag's, via ParentTag2ChildTag)
        Tag.parents.all()   (ParentTag2ChildTag's, via ParentTag2ChildTag)
    '''
...
@python_2_unicode_compatible
class ParentTag2ChildTag(CreatedBy):
    # A tag can have 0..n tags as children
    # related_name is the name for the reverse relationship.  
    # In this case, the reverse relationship is Tag referring to ParentTag2ChildTag's (as opposed to ParentTag2ChildTag referring to Tag)
    # e.g., 
    #   my_tag.children.all() returns the ParentTag2ChildTag's for my_tag (parent_tag=my_tag), representing the children of my_tag (where each child_tag is the child)
    ...
    class Meta:
        # Disallow duplicate ParentTag2ChildTag entries.
        unique_together = ('user', 'parent_tag', 'child_tag')

    def __str__(self):
        return f'ParentTag2ChildTag: parent_tag=[{self.parent_tag.name}] child_tag=[{self.child_tag.name}]'
